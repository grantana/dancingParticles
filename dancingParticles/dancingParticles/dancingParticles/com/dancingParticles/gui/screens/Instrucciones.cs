...
{
    internal class Instrucciones : Screen
    {


        Main main;

        public Instrucciones(Texture2D back, Texture2D rect, Main main)
        {
            clickableElements = new List<Button>();
            this.back = back;
            this.rect = rect;
            this.main = main;
            addButton(Properties.SCREEN_WITH - 180 + 10, 10, 50, 50, Properties.texturaBotonHome, 1);
            addButton(Properties.SCREEN_WITH - 180 + 90, 10, 50, 50, Properties.texturaBotonCerrar, 2);
        ...
        public void Update(Vector2 pos, MouseState mouse, GamePadState gps, GameTime gameTime)
        {
            ...
            if (mouse.LeftButton == ButtonState.Pressed || gps.Buttons.A == ButtonState.Pressed)
            {
                ...
                if (clickedID >= 0)
                {
                    ...
                    if (clickedID == 2)
                    {
                        main.stateManager.loadNextScreen(main.lastState, 0, gameTime);
                    }
